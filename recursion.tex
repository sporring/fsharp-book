...
\abstract{
  ...
  \item It must be a base case.
  \item The base case must be reached at some point, in order to avoid an infinite loop
  \end{enumerate}     
  In \Cref{fig:foreverRecursion} is an illustration the concept of an infinite loop with recursion.
...
A trick to designing recursive algorithms is to \advice{assume that the recursive function already works}. Reconsider the Divide-by-two algorith in \Cref{sec:divideByTwo}. The algorithm consists of two elements. Given an initial unsigned integer \lstinline{n}:
\begin{enumerate}
...
Consider Fibonacci's sequence of numbers which is a recursive sequence of numbers with relations to the Golden ratio and structures in biology. The Fibonacci sequence is the sequence of numbers $0, 1, 1, 2, 3, 5, 8, 13, \ldots$. The sequence starts with $0, 1$ and the next number is recursively given as the sum of the two previous ones. A direct implementation of this is given in \Cref{fibFor}.
%
