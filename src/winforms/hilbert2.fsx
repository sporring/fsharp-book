...
let model () : Size * ((Pen * (Point [])) list) = 
  ...
  let addRev (lst : Turtle list) (cmd : Command) (l : float) : Turtle list =
    let toInt = int << round
    match lst with
      | (p,d)::rest ->
        match cmd with
          | L -> (p, d + 3.141592/2.0)::rest // turn left
          | R -> (p, d - 3.141592/2.0)::rest // turn right
          | F -> // forward
            let (x, y) = (float p.X + l * cos d, float p.Y + l * sin d)
            (Point (toInt x, toInt y), d)::lst
