type vector(x : float, y : float) =
  ...
  static member multiply (v : vector, a : float) : vector =
    vector.multiply (a, v) // Symmetrize call §\label{vecCartesian2DOverloadSymmetrize}§
...
let pSym = vector.multiply (v, a) // Testing symmetry §\label{vecCartesian2DOverloadSymmetrizeApp}§
