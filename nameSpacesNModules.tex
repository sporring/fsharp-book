...
Every file implementation\jon{check} and script file in F\# implicitly defines a module, and the module name is given by the filename. Thus, creating a file \filename{Meta.fsx} with the following content:
%
...
\end{lstlisting}%$
\end{codeNOutput}
Libraries can of course be a compilation of any number of files into a single \filename{.dll} file. \filename{.dll}-files may be loaded dynamically in script files (\filename{.fsx}-files) using the \lstinline{#r} directive as illustrated below.
%
...
, and can cause and cannot be recommended
