...
F\# allows for assigning \idx{unit of measure} to \keyword{float}, \keyword{float32}, \keyword{decimal} and signed integer such as \keyword{int}.
%
\begin{lstlisting}[language=EBNF]
"[<Measure>] type" unit-name [ "=" measure ]
\end{lstlisting}
and then use \lstinline[language=EBNF]|"<" unit-name ">"| as suffix for literals. E.g., defining units of measure 'm' and 's', then we can make calculations like,
%
\begin{lstlisting}[language=fsharp,caption={fsharpi, floating point and integer numbers may be assigned units of measure.}]
> [<Measure>] type m
- [<Measure>] type s 
- let a = 3<m/s^2>
- let b = a * 10<s>
...
%
\begin{lstlisting}[language=fsharp,caption={fsharpi, units of measure adds an extra layer of types for syntax checking at compile time.}]
> [<Measure>] type m 
- [<Measure>] type s 
- let a = 1<m>
- let b = 1<s>
...
\end{lstlisting}
Units of measure allow for \token{*}, \token{/}, and \token{^}\jon{Spec-4.0: this notation is inconsistent with \texttt{**} for float exponentiation.} for multiplication, division and exponentiation. Values with units can be casted to \idx{unit-less} values by casting, and back again by multiplication as,
%
\begin{lstlisting}[language=fsharp,caption={fsharpi, type casting units of measure.}]
> [<Measure>] type m      
- let a = 2<m>            
- let b = int a           
- let c = b * 1<m>;;
...
%
\begin{lstlisting}[language=fsharp,caption={fsharpi, aggregated units of measure.}]
> [<Measure>] type s         
- [<Measure>] type m
- [<Measure>] type kg
...
%
\begin{lstlisting}[language=fsharp,caption={fsharpi, SI units of measure are built-in.}]
