...
%
meaning that a \lstinline[language=ebnf]!dDigit! is either \lstinline!0! or \lstinline!1!  or \dots or \lstinline!9!, an \lstinline[language=ebnf]!dInt! is one or more \lstinline[language=ebnf]!dDigit! such as \lstinline!3!, \lstinline!45!, and \lstinline!0124972930485738!.  A \lstinline[language=ebnf]!dFloat! is one or more digits, a dot and zero or more digits, such as \lstinline!0.4235!, \lstinline!3.!, but not \lstinline!.5! nor \lstinline!.!. There is no space between the digits and between digits and the dot.

Floating point numbers may alternatively be given using \idx{scientific notation}, such as \lstinline!3.5e-4! and \lstinline!4e2!, which means the number $3.5\cdot 10^{-4} = 0.00035$ and $4\cdot 10^2=400$. To describe this in EBNF we write
%
\begin{verbatimwrite}{tmp.ebnf}
...
int = dInt | xInt;
\end{verbatimwrite}
\fsebnf{tmp.ebnf}{Binary, heximal, and octal numbers.}
%
...
%   % 
%   \beverbatimwrite}{tmp.ebnf}
%     > let a = 5 - let b = -a;;
    
%     val a : int = 5 val b : int = -5
%   \end{verbatimwrite}
%\fsebnf{tmp.ebnf}{to do.}
% \item[\lstinline|leftOp + rightOp|, \lstinline|leftOp - rightOp|, \lstinline|leftOp * rightOp|:] These are binary operators, where addition, subtraction and multiplication performs the usual operations,
%   % 
%   \beverbatimwrite}{tmp.ebnf}
...
\lstinline{leftOp <<< rightOp}, which shifts the bit pattern of \lstinline|leftOp| \lstinline|rightOp| positions to the left insert 0's to right;
\lstinline{leftOp >>> rightOp}, which shifts the bit pattern of \lstinline|leftOp| \lstinline|rightOp| positions to the right insert 0's to left;
...
% \end{verbatimwrite}
%\fsebnf{tmp.ebnf}{to do.}
% \item[\lstinline|leftOp + rightOp|, \lstinline|leftOp - rightOp|, \lstinline|leftOp * rightOp|, \lstinline|leftOp / rightOp|:] These are binary operators, where addition, subtraction, multiplication, and division performs the usual operations, 
% %
% \begiverbatimwrite}{tmp.ebnf}
% > let a = 7.0 + 3.0
% - let b = 7.0 - 3.0
...
% \end{verbatimwrite}
%\fsebnf{tmp.ebnf}{to do.}
% \item[\lstinline|leftOp \% rightOp|:] 

% \item[\texttt{leftOp ** rightOp}:] In spite of an unusual notation, the binary exponentiation operator performs the usual calculation,
% %
% \begiverbatimwrite}{tmp.ebnf}
% > let a = 2.0 ** 5.0;;
