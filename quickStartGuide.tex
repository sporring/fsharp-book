...
% \end{lstlisting}
%
then apparently rebinding is valid. The difference is that the \idx{\lexeme{;;}} \idx{lexeme} defines a new nested \idx{scope}.\spec{Language change: Spec 4.0 p. 15.1 talks about blocks instead of scopes.} A lexeme is a letter or a word, which the F\# considers as an atomic unit. Scopes can be \idx[nested scope]{nested}, and in F\# a binding may reuse names in a nested scope, in which case the previous value is \idx[overshadow]{overshadowed}. I.e., attempting the same without \lexeme{;;} between the two let statements results in an error, e.g.,
%
...
\begin{figure}
  ...
  \caption{Binding of the the same name in the same scope is invalid in F\# 2, but valid in a different scopes. In (a) the two bindings are in the same scope, which is invalid, while in (b) the bindings are in separate scopes by the extra \lexeme{;;} lexeme, which is valid.}
  \label{fig:scope}
\end{figure}
