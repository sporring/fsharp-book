...
\item replace the mockup with real code while testing to keep track of your progress. Extend the unit-test as needed,
  ...
  \begin{tikzpicture} 
    \begin{package}{APackageName}
      \begin{class}[text width=3cm]{Base}{0,0} 
        ...
        \operation[0]{set: int -> ()} 
      \end{class}
      \begin{class}[text width=5cm]{Guest}{-4,-4}
        \attribute{aString : string}
        \operation{writeMessage : () -> ()}
      \end{class}
      \begin{class}[text width=5cm]{Derived}{4, -4}
        \inherit{Base} 
...
\begin{figure}
  ...
  \begin{tikzpicture} 
    \begin{package}{Race}
      \begin{class}[text width=10cm]{Game}{0,0} 
        \attribute{// racing clock}
        \attribute{time : int = 0}
        \operation{// setup game}
        \operation{// m human and n computer players}
        \operation{initialize:  (m : int) (n : int) -> () }
        \operation{// main loop, starts race and time, finishes when winner found}
        \operation{// or no players left. In each iteration clock is forwarded, each}
        \operation{// player is queried for their action, vehicles are updated,}
        \operation{// crashes are checked and handled, finish conditions are}
        \operation{// checked. }
        \operation{race:  () -> () }
      \end{class}
      \begin{class}[text width=7cm]{Player}{-5,-7.75} 
        \attribute{// number rounds completed}
      ...
      \end{class}
      \begin{class}[text width=7cm]{Vehicle}{0,-13} 
        ...
        \attribute{// handling ability 0.0..1.0, where 1.0 is best}
        ...
        \operation{crash :  () -> () }
      \end{class}
      \begin{class}[text width=7cm]{Track}{5,-7} 
        \attribute{// a curve specified by a list of coordinates}
        \attribute{shape : (int * int) list}
        \attribute{// constant width perpendicular to curve}
        \attribute{width : int}
        \attribute{// index into shape list}
        \attribute{startLine : int}
        \attribute{// index into shape list}
        \attribute{finishLine : int}
        \operation{// query whether a position is on or off track}
