...
\end{verbatimwrite}
\fsebnf{tmp.ebnf}{Expressions for controlling the flow of execution.}
...
expr = ... 
  ...
  | "for" ident "=" expr "to" expr "do" expr ["done"] (*simple for loop*)
\end{verbatimwrite}
\fsebnf{tmp.ebnf}{\lstinline{for}- and \lstinline{while}-loops.}
%
...
expr = ... 
  | "if" expr "then" expr {"elif" expr "then" expr} ["else" expr] (*conditional*)
\end{verbatimwrite}
\fsebnf{tmp.ebnf}{Conditional expressions.}
%
...
\end{verbatimwrite}
\fsebnf{tmp.ebnf}{Recursive functions.}
%
...
%
\fse{parity}{A better way to test for parity without recursion.}
%
