...
\abstract{ In the previous chapter, you got an introduction to the fundamental concepts of booleans, numbers, characters, and strings, how to perform calculations with them, and some of the limitations they have on the computer. This allows us to perform simple calculations as if the computer was an advanced pocket calculator. In this chapter, we will look at how we can:
  ...
  \item organise lines of code in functions, to make the same line reusable, and to make programs shorter, easier to understand.
  \item use operators as functions
  \item conditionally execute code
  \item how we can simulate the computer's execution of code by tracing-by-hand, to improve our understanding of, how it interprets the code, we write, and find errors.
  \end{itemize}
  Examples of problems, you can solve, after having read this chapter is:
  \begin{itemize}
...
%
Note that the expression \lstinline{3.0 ** p} must be put in parantheses here, to force F\# to \emph{first} calculate the result of the expression and \emph{then} give it to \lstinline{printfn} to be printed on the screen. The same expression in interactive mode will also show with the inferred types, as shown in \Cref{letValueLightWeightTypes}.
%
...
%% Jon 20220826: I don't think this is necesssary, and the type system does not support it well, except for classes
\begin{comment}
\section{Operators}
\label{sec:operators}
...
Programs often contain code which should only be executed under certain conditions. The \keyword{match} -- \keyword{with} expressions allows us to write such expressions, and its syntax is as follows:
%
...
Note that \keyword{match} -- \keyword{with} expressions will give a warning, if there are not cases for the full domain of what is being match. I.e., if we don't end with the wildcard pattern, when matching for integers, we would be missing cases for all other patterns
%
...
%
The condition \lstinline[language=syntax]{<*con*>} is an expression resulting in a Boolean value, and there can be zero or more \keyword{elif} conditions, as indicated by \lstinline[language=syntax]{{**}}. Each expression \lstinline[language=syntax]{<*expr*>}  is called a \idx{branch}, and all branches must have the same type, such that regardless of which branch is chosen, the type of the result of the conditional expression is the same. Then the expression of the first if-branch, whose condition is true, is evaluate. If all conditions are false then the \keyword{else}-branch is evaluated. If no \keyword{else} expression is present, then \lexeme{()} will be returned. See \Cref{weekdayIfThenElse} for a simple example.
%
\fs{weekdayIfThenElse}{Conditional computation with \keyword{if} -- \keyword{then}. Compare with \Cref{weekdayPatternGuard}.}
%
The branches are often several lines of code, in which case it is more useful to write them indendet, below the keywords, which may also make the code easier to read. The identical \lstinline{whatToDoToday} function with indendet branches is shown in \Cref{weekdayIfThenElseIndentation}.
%
...
\end{quote}
the value of \lstinline{a} are unqiuely determined, but F\# finds them to be erroneous, since F\# looks for the \keyword{else} to ensure all cases have been covered, and thatthe branches have the identical type. Hence,
\begin{quote}
...
This chapter we have taken a first look at organising code. Key concepts have been:
\begin{itemize}
