...
\abstract{
  ...
  \begin{itemize}
  \item Giving programs to ability to take arguments from the terminal,
  \item Read and write from to and from files,
  \item Handle errors by catching exceptions and if needed, throwing them yourself, and
...
In \Cref{exceptionDivByZero} \idx[dynamic type pattern]{dynamic type matching} is used using the \lexeme{:?} lexeme, i.e., the pattern matches exception with type \lstinline{System.DivideByZeroException} at runtime. The exception value may contain furter information and can be accessed if named using the \idx[as@\lstinline{as}]{\lstinline{as}}--keyword, as demonstrated in \Cref{exceptionDivByZeroNamed}.
%
